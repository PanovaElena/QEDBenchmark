...
#pragma omp simd
        ...
        )
        {
            ...
            int prevGeneratedParticleSize = generatedParticles.size();
            int prevGeneratedPhotonSize = generatedPhotons.size();

            // start avalanche with current photons
            for (int i = chunkBegin; i < chunkEnd; i++) {
                generatedPhotons.push_back(static_cast<Ensemble3d::ParticleType>(photons[i]));
            ...
            runAvalanche(timeStep,
                ...
                prevGeneratedParticleSize,
                generatedPhotons, timeAvalanchePhotons,
                eAvalanchePhotons, bAvalanchePhotons,
                prevGeneratedPhotonSize);

            // pop non-physical photons
            removeNonPhysicalPhotons(generatedPhotons, prevGeneratedPhotonSize);
        }
        ...
        )
        {
            ...
            rewriteParticlesInEnsemble(particles, generatedParticles,
                chunkBegin, prevGeneratedParticleSize, chunkEnd - chunkBegin);
        }
        ...
        void runAvalanche(FP timeStep,
            ...
            int generatedParticleStart,
            MyQEDVector<Ensemble3d::ParticleType>& generatedPhotons,
            MyQEDVector<FP>& timeAvalanchePhotons,
            MyQEDVector<FP3>& eAvalanchePhotons,
            MyQEDVector<FP3>& bAvalanchePhotons,
            int generatedPhotonStart)
        {
            int countProcessedParticles = generatedParticleStart;
            int countProcessedPhotons = generatedPhotonStart;
                ...
                || countProcessedPhotons != generatedPhotons.size())
            {
                ...
                for (int chunkIndex = 0; chunkIndex < nChunks; chunkIndex++)
                {
                    ...
                    oneParticleStep(timeStep, generatedParticleStart,
                        generatedParticles, timeAvalancheParticles,
                        eAvalancheParticles, bAvalancheParticles,
                        generatedPhotons, timeAvalanchePhotons,
                        eAvalanchePhotons, bAvalanchePhotons,
                ...
                for (int chunkIndex = 0; chunkIndex < nChunks; chunkIndex++)
                {
                    ...
                    onePhotonStep(timeStep, generatedPhotonStart,
                        generatedPhotons, timeAvalanchePhotons,
                        eAvalanchePhotons, bAvalanchePhotons,
                        generatedParticles, timeAvalancheParticles,
                        eAvalancheParticles, bAvalancheParticles,
        ...
        void oneParticleStep(FP timeStep, int generatedParticleStart,
            ...
            int begin, int end)
        {
            begin = sortHandledParticles(particles, timeStep,
                time, e, b, begin, end, generatedParticleStart);
            ...
            while (begin < end) {
                ...
                int s = begin, sAux = begin - generatedParticleStart;
                int size = end - begin;
...
#pragma omp simd
                for (int i = 0; i < size; i++) {
                    ...
                    FP H_eff = sqr(e[sAux + i] + ((FP)1 / Constants<FP>::lightVelocity()) * VP(v, b[sAux + i]))
                        - sqr(SP(e[sAux + i], v) / Constants<FP>::lightVelocity());
                    if (H_eff < 0) H_eff = 0;
                    H_eff = sqrt(H_eff);
...
#pragma omp simd
                for (int i = 0; i < size; i++) {
                    bool cond = dt[i] + time[sAux + i] > timeStep;
                    FP realDt = cond ? timeStep - time[sAux + i] : dt[i];
                    boris(particles[s + i], e[sAux + i], b[sAux + i], realDt);
                    time[sAux + i] = cond ? timeStep : time[sAux + i] + dt[i];
                }

                // eliminate finished particles
                int newBegin = sortHandledParticles(particles, timeStep, time, e, b,
                    begin, end, generatedParticleStart, chi);
                int elimShift = newBegin - begin;
                begin = newBegin;
                s = begin;
                sAux = begin - generatedParticleStart;
                size = end - begin;
...
#pragma omp simd
                ...
                timeAvalanchePhotons.extend(time.data() + sAux, size);
                eAvalanchePhotons.extend(e.data() + sAux, size);
                bAvalanchePhotons.extend(b.data() + sAux, size);
...
#pragma omp simd
        ...
        void onePhotonStep(FP timeStep, int generatedPhotonStart,
            ...
            int begin, int end)
        {
            ...
            int s = begin, sAux = begin - generatedPhotonStart;
...
#pragma omp simd
            for (int i = 0; i < size; i++) {
                ...
                FP H_eff = sqrt(sqr(e[sAux + i] + VP(k_[i], b[sAux + i])) - sqr(SP(e[sAux + i], k_[i])));
...
#pragma omp simd
            ...
            int newBegin = sortHandledParticles(photons, timeStep, time, e, b,
                begin, end, generatedPhotonStart, chi);
            int elimShift = newBegin - begin;
            size -= elimShift;
            begin = newBegin;
            s = begin;
            sAux = begin - generatedPhotonStart;
...
#pragma omp simd
            ...
            timeAvalancheParticles.extend(time.data() + sAux, size);
            eAvalancheParticles.extend(e.data() + sAux, size);
            bAvalancheParticles.extend(b.data() + sAux, size);
...
#pragma omp simd
        ...
        forceinline int sortHandledParticles(MyQEDVector<Ensemble3d::ParticleType>& particles,
            FP timeStep, MyQEDVector<FP>& vtime, MyQEDVector<FP3>& ve, MyQEDVector<FP3>& vb,
            int begin, int end, int shift, FP* const chi = nullptr)
        {
            int lastIndex = begin;
            for (int i = begin; i < end; i++)
                if (vtime[i - shift] >= timeStep) {
                    if (lastIndex != i) {
                        std::swap(particles[i], particles[lastIndex]);
                        std::swap(vtime[i - shift], vtime[lastIndex - shift]);
                        std::swap(ve[i - shift], ve[lastIndex - shift]);
                        std::swap(vb[i - shift], vb[lastIndex - shift]);
        ...
        forceinline void rewriteParticlesInEnsemble(
            ...
            MyQEDVector<Ensemble3d::ParticleType>& generatedParticles,
            int beginParticles, int beginGeneratedParticles, int size)
        {
            // replace old particles with new particles
            for (int i = 0; i < size; i++)
                particles[beginParticles + i] = generatedParticles[beginGeneratedParticles + i];
            // pop double particles
            int nMove = generatedParticles.size() - (beginGeneratedParticles + size) <= size ?
                generatedParticles.size() - (beginGeneratedParticles + size) : size;
            for (int j = 0; j < nMove; j++)
                std::swap(generatedParticles[j + beginGeneratedParticles],
