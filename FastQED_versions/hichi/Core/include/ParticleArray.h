...
namespace pfc {
    ...
    public:
        ...
        {
            swap(it1.getIdx(), it2.getIdx());
        }

        inline void swap(int idx1, int idx2)  // !!!!!!!!!!!!!!!
        {
            for (int d = 0; d < positionDimension; d++)
                std::swap(positions[d][idx1], positions[d][idx2]);
            for (int d = 0; d < momentumDimension; d++)
                std::swap(ps[d][idx1], ps[d][idx2]);

            std::swap(weights[idx1], weights[idx2]);
            std::swap(gammas[idx1], gammas[idx2]);
        }
