...
for tmom in range(100, 101, 100):  
    ...
    colors=['red', 'blue', 'green', 'orange', 'magenta', 'yellow', 'pink', 'cyan', 'brown', 'grey', 'purple', 'olive'] * 2
    
    fig = plt.figure(figsize=(15,5.5))
    canvas = FigureCanvas(fig)
    ax1=fig.add_subplot(1, 4, 1) #fig.add_axes([0.03,0.15,0.28,0.8])
    ax1.set_yscale('log')
    ax2=fig.add_subplot(1, 4, 2) #fig.add_axes([0.35,0.15,0.28,0.8])
    ax3=fig.add_subplot(1, 4, 3) #fig.add_axes([0.68,0.15,0.28,0.8])
    ...
    for isim in range(nsims):
        ...
        ax1.text(0.3, 0.95-isim*0.05, r'$\Gamma_{%d}T = %.6g$' % (isim,params[1]), fontsize=14,transform=ax1.transAxes)
    ...
    canvas.print_figure('Compare%d.png' % (tmom), dpi=400)
