    ...
    public:
        ...
        void processParticles(Ensemble3d* particles, TGrid* grid, FP timeStep)
        {
            ...
            for (int th = 0; th < max_threads; th++)
            {
                ...
                {
                    //FP a = 4e-5;
                    //int sector = (int)(afterAvalanchePhotons[th][ind].getPosition().x * 2.0 / a + 2.0);
                    //std::cout << "added photon in " << sector << std::endl;
...
//#pragma omp parallel for schedule(dynamic, 1)
        ...
        void HandleParticles(ParticleArray3d& particles, TGrid* grid, FP timeStep)
        {
            ...
            int counter = 0;
            for (int i = 0; i < particles.size(); i++) {
                if (particles[i].getMomentum().norm2() == 0) counter++;
            }
            ...
            for (int i = 0; i != NT; i++)
            {
                for (int k = 0; k != AvalancheParticles.size(); k++)
                {
                    ...
                    if (delta != 0)
                    {
                        ...
                        if (NewParticle.getMomentum().norm2() == 0.0) {  // !!!!!!!!
                            std::cout << "Alarm! momentum=0! " << AvalancheParticles[k].getMomentum() << std::endl;
                        }
